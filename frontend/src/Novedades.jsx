...
function Novedades() {
  ...
  const cargarListaEmpleados = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${backendUrl}/empleados?limit=all`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const enviarQueja = async (e) => {
    ...
    try {
      ...
      await axios.post(`${backendUrl}/quejas`, {
        tipo: tipoIncidencia,
  ...
  const eliminarReporte = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${backendUrl}/quejas/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 px-2 sm:px-4">
      ...
      <div className="bg-slate-800 rounded-xl p-4 sm:p-8 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button 
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg font-bold transition shadow flex items-center gap-2 text-xs sm:text-sm whitespace-nowrap w-full md:w-auto justify-center"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-4 sm:p-6">
            <h3 className="text-lg sm:text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">📢 Reportar Incidencia</h3>
            <form onSubmit={enviarQueja} className="space-y-4">
              ...
              <div>
                ...
                <select 
                  className="w-full p-2.5 border rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 bg-white font-medium disabled:bg-gray-100" 
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Buscar Empleados Involucrados (Opcional):</label>
                <input 
                  type="text"
                  ...
                  disabled={cargando}
                  className="w-full p-2.5 border rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                />
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {empleadosSeleccionados.map(emp => (
                    <span 
                      key={emp.empleadoid || emp.empleadoID} 
                      className="bg-blue-100 text-blue-800 text-[11px] sm:text-xs font-bold px-2.5 py-1 rounded-full flex items-center gap-2 border border-blue-300"
                    >
                  ...
                  {empleadosSeleccionados.length === 0 && (
                    <p className="text-[11px] text-slate-400 italic">Ningún empleado seleccionado.</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <textarea 
                  required 
                  className="w-full p-3 border rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 h-24 resize-none disabled:bg-gray-100" 
              ...
              <button 
                type="submit" 
                disabled={cargando}
                className="w-full bg-slate-800 hover:bg-black text-white font-bold py-2.5 rounded-lg transition shadow disabled:opacity-50 text-xs sm:text-sm"
              >
          ...
          <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-4 sm:p-6">
            <h3 className="text-lg sm:text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">📋 Últimos Reportes</h3>
            <div className="max-h-60 overflow-y-auto pr-1 space-y-3">
              {quejas.length === 0 ? (
                <p className="p-4 bg-slate-50 text-slate-500 text-center rounded italic border border-dashed text-xs sm:text-sm">Sin novedades recientes.</p>
              ) : (
                quejas.map((q, index) => (
                  <div key={q.quejaid || index} className="p-3 border-l-4 border-blue-500 bg-slate-50 rounded shadow-sm flex justify-between items-start">
                    <div className="space-y-1 pr-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[10px] sm:text-xs font-bold text-blue-700 uppercase bg-blue-50 px-2 py-0.5 rounded border border-blue-200">{q.tipo}</span>
                        <span className="text-[10px] sm:text-xs text-slate-400 font-mono">{q.fechaqueja ? new Date(q.fechaqueja).toLocaleDateString('es-VE') : 'Hoy'}</span>
                      </div>
                      <p className="text-xs sm:text-sm text-slate-700 mt-1">{q.descripcion}</p>
                    </div>
